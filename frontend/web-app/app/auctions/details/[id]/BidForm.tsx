...
export default function BidForm({ auctionId, highBid }: Props) {
    ...
    const onSubmit = (data: FieldValues) => {
        if(data.amount <= highBid) {
            reset();
            return toast.error('Предложение должно быть больше ' + numberWithCommas(highBid) + ' руб');
        }
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className='flex items-center border-2 rounded-lg py-2'>
            ...
            placeholder={`Укажите ваше предложение (минимально - ${numberWithCommas(highBid + 1)}) руб`}
            />
        </form>
    )
}
